...
class ActiveSessionNotifier extends Notifier<ActiveSessionState> {
  ...
  void _processFirebaseTelemetry(DeviceInfo deviceInfo, AudioService audioService) {
    ...
    bool nuevaCompresion = rawCompressions > (state.lastDeviceInfo?.compresiones ?? 0);

    // Logging de telemetría para depuración (traceability)
    debugPrint('[Telemetry] MAC: ${deviceInfo.macAddress} | Raw CP: $rawCompressions | Correct: ${deviceInfo.compresionesCorrectas} | isActive: ${deviceInfo.isActive}');
    ...
    final trimmed = newHistory.length > 40 ? newHistory.sublist(newHistory.length - 40) : newHistory;
  ...
  Future<SessionModel> endSession() async {
    ...
    debugPrint('[Session] Finalizando sesión... esperando último pulso de datos.');
    ...
    final metrics = _computeSessionMetrics(
      state.lastDeviceInfo ?? DeviceInfo(macAddress: '', fuerzaKg: 0, profundidadMm: 0, frecuenciaCpm: 0, compresiones: 0, compresionesCorrectas: 0, recoilOk: true, enCompresion: false, compresionCorrecta: true, calidadPct: 0, recoilPct: 0, avgProfundidadMm: 0, avgFuerzaKg: 0, pausas: 0, maxPausaSeg: 0, sensorOk: true, calibrado: false, timestamp: 0, isActive: false),
  ...
  SessionMetrics _computeSessionMetrics(DeviceInfo devInfo, DeviceInfo? initial, Duration elapsed) {
    ...
    debugPrint('[Metrics] Final Calc: Raw(${devInfo.compresiones}) - Init(${initial?.compresiones ?? 0}) = $count CP');
    
    int correctCount = devInfo.compresionesCorrectas - (initial?.compresionesCorrectas ?? 0);
    ...
    if (count > 0) {
      final double initialDepthSum = (initial?.avgProfundidadMm ?? 0.0) * initialCount;
      ...
      final double initialFuerzaSum = (initial?.avgFuerzaKg ?? 0.0) * initialCount;
      ...
      final double initialRecoilSum = ((initial?.recoilPct ?? 100.0) / 100.0) * initialCount;
