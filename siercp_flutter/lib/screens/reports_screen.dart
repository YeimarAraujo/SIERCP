...
class _ReportsScreenState extends ConsumerState<ReportsScreen>
              ...
              child: Row(
                children: [
                  ...
                  Column(
                    ...
                    children: [
                      Text('Reportes', style: TextStyle(color: textP, fontSize: 20, fontWeight: FontWeight.w700)),
                      Text('Genera y consulta reportes PDF', style: TextStyle(color: textS, fontSize: 12)),
                    ],
                  ),
                ],
              ),
  ...
  Future<void> _handleGenerate(String type, {
    String? courseId,
    String? courseName,
    String? studentId,
    String? studentName,
    ...
    try {
      ...
      if (type == 'course' && courseId != null) {
        ...
        final course = localSvc.getCourse(courseId) ??
            CourseModel(id: courseId, title: courseName ?? '', instructorName: '', totalModules: 0, completedModules: 0, certification: '');
...
class _GenerateTab extends ConsumerWidget {
  ...
  final Future<void> Function(String type, {String? courseId, String? courseName, String? studentId, String? studentName}) onGenerate;
        ...
        if (courses.isEmpty) {
          return Center(
            child: Column(mainAxisSize: MainAxisSize.min, children: [
              Icon(Icons.folder_off_outlined, size: 48, color: textS.withValues(alpha: 0.3)),
              const SizedBox(height: 12),
              Text('No hay cursos disponibles', style: TextStyle(color: textS, fontSize: 13)),
            ]),
          );
        }
        ...
        return ListView.builder(
          ...
          itemBuilder: (ctx, i) {
            ...
            return _CourseReportCard(
              ...
              onGenerateCourse: () => onGenerate('course', courseId: course.id, courseName: course.title),
              onGenerateStudent: (sid, sname) => onGenerate('student',
                  courseId: course.id, courseName: course.title, studentId: sid, studentName: sname),
            );
          },
        );
      },
...
class _CourseReportCard extends ConsumerStatefulWidget {
  ...
  const _CourseReportCard({
    required this.course, required this.isDark, required this.surface,
    required this.border, required this.textP, required this.textS,
    required this.generating, required this.canGenerate,
    required this.onGenerateCourse, required this.onGenerateStudent,
  });
  ...
  Widget build(BuildContext context) {
    return AnimatedContainer(
      ...
      decoration: BoxDecoration(
        ...
        border: Border.all(color: widget.border.withValues(alpha: 0.4), width: 0.5),
            ...
            child: Padding(
              ...
              child: Row(children: [
                ...
                Expanded(child: Column(
                  crossAxisAlignment: CrossAxisAlignment.start,
                  children: [
                    Text(widget.course.title, style: TextStyle(color: widget.textP, fontSize: 14, fontWeight: FontWeight.w600)),
                    Text('${widget.course.studentCount ?? 0} estudiantes', style: TextStyle(color: widget.textS, fontSize: 11)),
                  ],
                )),
                if (widget.canGenerate)
                  widget.generating
                      ...
                      : IconButton(
                          icon: const Icon(Icons.picture_as_pdf_rounded, color: AppColors.brand, size: 20),
                          tooltip: 'Reporte consolidado del curso',
                          onPressed: widget.onGenerateCourse,
                        ),
                Icon(_expanded ? Icons.expand_less : Icons.expand_more, color: widget.textS, size: 20),
              ]),
            ),
          ),
...
class _StudentsList extends ConsumerWidget {
  ...
  const _StudentsList({
    required this.courseId, required this.textP, required this.textS,
    ...
    return studentsAsync.when(
      ...
      data: (students) {
        if (students.isEmpty) {
          return Padding(
            padding: const EdgeInsets.all(16),
            child: Text('Sin estudiantes inscritos', style: TextStyle(color: textS, fontSize: 12)),
          );
        }
        return Column(
          children: [
              ...
              return ListTile(
                ...
                leading: CircleAvatar(
                  ...
                  child: Text(
                    ...
                    style: const TextStyle(color: AppColors.brand, fontSize: 12, fontWeight: FontWeight.w700),
                  ),
                ),
                title: Text(name, style: TextStyle(color: textP, fontSize: 12, fontWeight: FontWeight.w500)),
                subtitle: Text('Promedio: ${avg.toStringAsFixed(1)}%', style: TextStyle(color: textS, fontSize: 10)),
                trailing: generating
                    ? const SizedBox(width: 16, height: 16, child: CircularProgressIndicator(strokeWidth: 1.5, color: AppColors.brand))
                    : IconButton(
                        icon: const Icon(Icons.description_outlined, color: AppColors.cyan, size: 18),
...
class _SavedReportsTab extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    if (reports.isEmpty) {
      return Center(
        child: Column(mainAxisSize: MainAxisSize.min, children: [
          Icon(Icons.folder_open_outlined, size: 52, color: textS.withValues(alpha: 0.3)),
          const SizedBox(height: 12),
          Text('No hay reportes guardados', style: TextStyle(color: textS, fontSize: 13)),
          const SizedBox(height: 4),
          Text('Genera uno desde la pestaña anterior', style: TextStyle(color: textS.withValues(alpha: 0.6), fontSize: 11)),
        ]),
      );
    }
    ...
    return RefreshIndicator(
      ...
      child: ListView.builder(
        ...
        itemBuilder: (ctx, i) {
          ...
          final color = r.type == 'course' ? AppColors.brand : AppColors.cyan;
          final icon = r.type == 'course' ? Icons.groups_outlined : Icons.person_outline;
          ...
          return Container(
            ...
            child: ListTile(
              ...
              title: Text(r.title, style: TextStyle(color: textP, fontSize: 13, fontWeight: FontWeight.w600)),
              subtitle: Text(
                '${DateFormat('dd MMM yyyy · HH:mm').format(r.generatedAt)} · ${r.sessionCount} sesiones',
              ...
              trailing: Row(mainAxisSize: MainAxisSize.min, children: [
                if (r.averageScore != null)
                  Container(
                    ...
                    decoration: BoxDecoration(
                      color: (r.averageScore! >= 70 ? AppColors.green : AppColors.red).withValues(alpha: 0.12),
                      borderRadius: BorderRadius.circular(20),
                    ),
                    child: Text('${r.averageScore!.toStringAsFixed(0)}%',
                        style: TextStyle(color: r.averageScore! >= 70 ? AppColors.green : AppColors.red, fontSize: 11, fontWeight: FontWeight.w700)),
                  ),
                ...
                PopupMenuButton<String>(
                  ...
                  onSelected: (v) async {
                    if (v == 'open' && exists) {
                      ...
                      await Share.shareXFiles([XFile(r.filePath, mimeType: 'application/pdf')],
                          subject: r.title);
                    } else if (v == 'delete') {
                      await localSvc.deleteReport(r.id);
                      if (exists) try { await File(r.filePath).delete(); } catch (_) {}
                      if (ctx.mounted) (ctx as Element).markNeedsBuild();
                    }
                  },
                  itemBuilder: (_) => [
                    if (exists) const PopupMenuItem(value: 'open', child: Row(children: [
                      Icon(Icons.open_in_new, size: 16, color: AppColors.brand),
                      SizedBox(width: 8), Text('Abrir'),
                    ])),
                    if (exists) const PopupMenuItem(value: 'share', child: Row(children: [
                      Icon(Icons.share_outlined, size: 16, color: AppColors.cyan),
                      SizedBox(width: 8), Text('Compartir'),
                    ])),
                    const PopupMenuItem(value: 'delete', child: Row(children: [
                      Icon(Icons.delete_outline, size: 16, color: AppColors.red),
                      SizedBox(width: 8), Text('Eliminar'),
                    ])),
                  ],
                ),
              ]),
